    ...
    if request.method == 'POST':
        ...
        if error is None:
            try:
                ...
                db.execute(
                    "INSERT INTO developer (email, password, permission_id) VALUES (?, ?, ?)",
...
def login():
    if request.method == 'POST':
        ...
        error = None
        user = db.execute(
            'SELECT id, email, password FROM developer WHERE email = ?', (email,)
        ).fetchone()
...
def load_logged_in_user():
    ...
    if user_id is None:
        ...
        g.user = get_db().execute(
            'SELECT id, email, password, permission_id FROM developer WHERE id = ?', (user_id,)
        ).fetchone()
...
def accounts():
    #nur admins!
    if g.user['permission_id'] == 1:
        db = get_db()
        #list of tuples
        accounts = db.execute(
            'SELECT T_Developer.id, T_Developer.email, T_Developer.password, T_Developer.permission_id, T_Permission.id, T_Permission.permission_name FROM developer T_Developer INNER JOIN permission T_Permission ON T_Developer.permission_id = T_Permission.id ORDER BY T_Developer.id'
        ).fetchall()
...
def account_check()->bool:
    ...
    if g.user['permission_id'] == 1:
        ...
        if request.method == 'GET':
            ...
            account = db.execute(
                f'SELECT id, email, permission_id FROM developer WHERE id = {user_form_id}'
            ).fetchone()
            ...
            try:
                db = get_db()
                db.execute(f"UPDATE developer SET email = '{user_form_mail}', permission_id = {user_form_permission_id} WHERE id = {user_form_id}")
                db.commit()
            ...
            try:
                db = get_db()
                db.execute(f"DELETE FROM developer WHERE id = {user_form_id}")
                db.commit()
