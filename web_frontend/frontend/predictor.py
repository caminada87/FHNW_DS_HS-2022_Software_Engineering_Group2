...
class HousePricePrediction(Resource):
    def __init__(self) -> None:
        """
        Initialize class values, get data from user-interface.
        """
        self.filename: str = current_app.config["MODEL"]
        self.parser = reqparse.RequestParser()
        self.parser.add_argument(
            "longitude", type=float, location="args", default=0.0
        )
        self.parser.add_argument(
            "latitude", type=float, location="args", default=0.0
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "bulding_category", type=str, location="args",
            default="Einfamilienhaus"
        )
        self.parser.add_argument(
            "build_year", type=int, location="args", default=2000
        )
        self.parser.add_argument(
            "living_area", type=float, location="args", default=200.0
        )
        self.parser.add_argument(
            "num_rooms", type=float, location="args", default=7
        )
        self.parser.add_argument(
            "user_id", type=int, location="args", default=0
        )

        with open(self.filename, "rb") as pickle_file:
            self.model = pickle.load(pickle_file)

    def get(self) -> tuple[str, int] | tuple[Response, int]:
        """
        Get all predictions made for the input.
        """
        predictions = []
        ...
        predictions_db = db.execute(
            ...
            "FROM predictions WHERE query_data = ?",
            (request_json,),
        ).fetchall()
    ...
    def put(self) -> Response:
        """
        Make prediction for user values and save it in database.
        @return: prediction price as JSON
        """
        args = self.parser.parse_args()
