...
class HousePricePrediction(Resource):
    def __init__(self):
        ...
        self.parser.add_argument('longitude', type=float, location='args',
                                 default=0.0)
        self.parser.add_argument('latitude', type=float, location='args',
                                 default=0.0)
        self.parser.add_argument('postal_code', type=int, location='args',
                                 default=5420)
        self.parser.add_argument('city', type=str, location='args',
                                 default='Ehrendingen')
        self.parser.add_argument('bulding_category', type=str, location='args',
                                 default='Einfamilienhaus')
        self.parser.add_argument('build_year', type=int, location='args',
                                 default=2000)
        self.parser.add_argument('living_area', type=float, location='args',
                                 default=200.0)
        self.parser.add_argument('num_rooms', type=float, location='args',
                                 default=7)
    ...
    def get(self):
        ...
        predictions_db = db.execute(
            'SELECT id, user_id, query_data, predicted_price FROM predictions WHERE query_data = ?',
            (request_json,)
        ).fetchall()
    ...
    def put(self):
        ...
        db = get_db()
        db.execute(
            f"INSERT INTO predictions (user_id, query_data, predicted_price) VALUES (?,?,?)",
